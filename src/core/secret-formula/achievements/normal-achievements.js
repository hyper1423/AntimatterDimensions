...
export const normalAchievements = [
  ...
  },
  {
    ...
    get description() {
      return `Get all Antimatter Dimension multipliers over ${formatX(Decimal.dNumberMax, 1)}.`;
    },
    checkRequirement: () => AntimatterDimensions.all.every(x => x.multiplier.gte(Decimal.dNumberMax)),
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
  ...
  },
  {
    ...
    get description() {
      return `Get a ${formatX(Decimal.dNumberMax, 1, 0)} multiplier in a single Dimensional Sacrifice.`;
    },
    checkRequirement: () => Sacrifice.nextBoost.gte(Decimal.dNumberMax),
  ...
  },
  {
    id: 95,
    name: "Is this safe?",
    get description() { return `Gain ${format(Decimal.dNumberMax, 1, 0)} Replicanti in ${formatInt(1)} hour.`; },
    get reward() { return `You keep your Replicanti and ${formatInt(1)} Replicanti Galaxy on Infinity.`; },
    checkRequirement: () =>
      (Replicanti.amount.eq(Decimal.dNumberMax) || player.replicanti.galaxies > 0) &&
  ...
  },
  {
    ...
    get description() {
      ...
      ${format(Decimal.dNumberMax, 1, 0)} times higher Infinity Points than the previous one.`;
    },
    checkRequirement: () => {
      ...
      for (let i = 0; i < infinities.length - 1; i++) {
        if (infinities[i].lt(infinities[i + 1].times(Decimal.dNumberMax))) return false;
      }
      return true;
    },
    checkEvent: GAME_EVENT.BIG_CRUNCH_AFTER,
  ...
  },
  {
    ...
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
    get reward() {
      return `Replicanti Galaxies divide your Replicanti by ${format(Decimal.dNumberMax, 1, 0)}
  ...
  },
  {
    ...
    get description() { return `Reach ${format(Decimal.dNumberMax, 1, 0)} Eternity Points.`; },
    checkRequirement: () => Currency.eternityPoints.gte(Decimal.dNumberMax),
    checkEvent: GAME_EVENT.GAME_TICK_AFTER
  },
  {
    id: 128,
  ...
  },
  {
    ...
    get description() {
      ...
      ${format(Decimal.dNumberMax, 1, 0)} times higher Eternity Points than the previous one.`;
    },
    checkRequirement: () => {
      ...
      for (let i = 0; i < eternities.length - 1; i++) {
        if (eternities[i].lt(eternities[i + 1].times(Decimal.dNumberMax))) return false;
      }
      return true;
    },
    checkEvent: GAME_EVENT.ETERNITY_RESET_AFTER,
  ...
  },
  {
    ...
    get description() { return `Get ${format(Decimal.dNumberMax, 1)} Infinities.`; },
    checkRequirement: () => Currency.infinitiesTotal.gte(Decimal.dNumberMax),
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
  ...
  },
  {
    ...
    get description() { return `Reach ${format(Decimal.dNumberMax, 1, 0)} Reality Machines.`; },
    checkRequirement: () => Currency.realityMachines.gte(Decimal.dNumberMax),
    checkEvent: GAME_EVENT.GAME_TICK_AFTER,
  ...
  },
  {
    ...
    get description() {
      return `Reality for ${format(Decimal.dNumberMax, 1)} Reality Machines without having
      any Charged Infinity Upgrades, having any equipped Glyphs, or buying any Triad Studies.`;
    },
    checkRequirement: () => MachineHandler.gainedRealityMachines.gte(Decimal.dNumberMax) &&
