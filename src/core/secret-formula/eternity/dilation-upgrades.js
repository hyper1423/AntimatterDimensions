...
export const dilationUpgrades = {
  ...
  tdMultReplicanti: {
    ...
    description: () => {
      ...
      let multiplier = "0.1";
      if (rep10 > 9000) {
        const ratio = DilationUpgrade.tdMultReplicanti.effectValue.pLog10().div(rep10).toNumber();
        if (ratio < 0.095) {
  ...
  flatDilationMult: {
    ...
    effect: () => Decimal.pow(1e9, Decimal.min((Decimal.max(player.eternityPoints.log10().minus(1500), 0).div(2500)).pow(1.2), 1)),
