...
export const Effarig = {
  ...
  get shardsGained() {
    ...
    return Decimal.floor(Decimal.pow(Currency.eternityPoints.value.spLog10().floor().div(7500), this.glyphEffectAmount))
      .times(AlchemyResource.effarig.effectValue);
  },
  get maxRarityBoost() {
    return Decimal.log10(Decimal.log10(Currency.relicShards.value.plus(10))).times(5).toNumber();
  },
  nerfFactor(power) {
    let c;
    switch (this.currentStage) {
    ...
    return Decimal.times(3, Decimal.div(1 - c, Decimal.clampMin(0.0005, Decimal.sqrt(power.eq(0) ? DC.D0 : power.pLog10()).plus(c)))).toNumber();
  },
  get tickDilation() {
  ...
  get tickspeed() {
    const base = Decimal.plus(3, Tickspeed.baseValue.reciprocal().log10());
