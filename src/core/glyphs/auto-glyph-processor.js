...
function getGlyphLevelSources() {
  ...
  const epCoeff = 0.016;
  const epBase = Decimal.pow(
      Decimal.max(1, eternityPoints.pLog10()), 
      0.5
    )
    .times(epCoeff)
    .toNumber();
  ...
  const replBase = Decimal.pow(
      Decimal.max(1, player.records.thisReality.maxReplicanti.log10()), 
      replPow
    )
    .times(replCoeff)
    .toNumber();
  ...
  const dtBase = Decimal.pow(
      Decimal.max(1, player.records.thisReality.maxDT.pLog10()), 
      dtPow
    )
    .times(dtCoeff)
    .toNumber();
