...
export default {
  ...
  computed: {
    ...
    amountStyle() {
      ...
      const ratio = this.gainedEP.log10().div(this.currentEP.log10()).toNumber();
      const interFn = index => {
  ...
  methods: {
    update() {
      ...
      this.currentEPRate.copyFrom(gainedEP.dividedBy(
        Math.clampMin(0.0005, TimeSpan.fromMilliseconds(player.records.thisEternity.realTime).totalMinutes)));
