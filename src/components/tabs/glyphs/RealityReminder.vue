...
export default {
  ...
  methods: {
    update() {
      ...
      this.currLog10EP = player.eternityPoints.log10().toNumber();
      this.cheapestLog10TD = Decimal.min(...TimeDimensions.all.map(x => x.cost.log10())).toNumber();
      this.multEPLog10Cost = EternityUpgrade.epMult.cost.log10().toNumber();
